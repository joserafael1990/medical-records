...
def check_consultation():
    try:
        ...
        cur.execute("""
            SELECT id, patient_id, doctor_id, consultation_date, prescribed_medications, treatment_plan, created_at, updated_at
        ...
        if consultation:
            ...
            print(f"  Created At: {consultation[6]}")
            print(f"  Updated At: {consultation[7]}")
        else:
            print("❌ Consultation ID 31 not found")
        
        # Check recent consultations
        cur.execute("""
            SELECT id, patient_id, doctor_id, consultation_date, prescribed_medications, created_at
            FROM medical_records 
