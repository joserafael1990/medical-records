...
def upgrade() -> None:
    ...
    # Create appointment_reminders table if it doesn't exist (needed for fresh databases)
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_appointment_reminders_appointment_id ON appointment_reminders(appointment_id)")
    op.execute("CREATE INDEX IF NOT EXISTS idx_appointment_reminders_appointment_status ON appointment_reminders(appointment_id) WHERE enabled = TRUE AND sent = FALSE")
    op.execute("CREATE INDEX IF NOT EXISTS idx_appointment_reminders_enabled_sent ON appointment_reminders(enabled, sent) WHERE enabled = TRUE AND sent = FALSE")
    op.execute("CREATE INDEX IF NOT EXISTS idx_appointment_reminders_whatsapp_message_id ON appointment_reminders(whatsapp_message_id)")
    op.execute("COMMENT ON TABLE appointment_reminders IS 'Stores up to 3 automatic reminders per appointment. Each reminder can be enabled/disabled independently and has a custom offset_minutes (time before appointment).'")
