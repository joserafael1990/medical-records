...
export const AvatarManagerDialog: React.FC<AvatarManagerDialogProps> = ({
      ...
      return (
        <Grid container spacing={2} sx={{ mt: 1 }}>
          {preloaded.map((option) => (
            <Grid size={{ xs: 4, sm: 3, md: 2 }} key={option.templateKey ?? option.key}>
              <AvatarGridItem
                option={option}
    ...
    return (
      <Box sx={{ mt: 1 }}>
        ...
        <Grid container spacing={2}>
          {custom.map((option) => (
            <Grid size={{ xs: 4, sm: 3, md: 2 }} key={option.relativePath ?? option.relative_path}>
              <AvatarGridItem
                option={option}
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth aria-labelledby="avatar-manager-title">
      <DialogTitle id="avatar-manager-title">Cambiar avatar</DialogTitle>
